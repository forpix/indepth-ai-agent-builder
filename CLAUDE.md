# CLAUDE.md — Agent Builder Demo 项目上下文

> 这份文件是 Claude Code 的项目持久上下文。每次会话开始时自动加载，作为所有产品和工程决策的依据。
> 用途：B2B Agent 编排工作台作品集 Demo（Skill Builder + Agent Console + Debug & Eval）
...
3. 如果当前任务涉及的 spec 文件**不存在**或**不完整**，**立即停下来询问 Maintainer**，不要根据项目名脑补需求

**违反这个协议会浪费时间，且产出的代码很可能要返工。**
...
一个面向 **B2B AI 产品经理岗位**的作品集 Demo —— B2B Agent 编排工作台。

围绕"制造业采购交期跟催"这一具体业务场景，展示如何用低代码搭出可调试、可复用、可监控的 B2B Agent，以及配套的运行时控制台和 Trace / 指标评估能力。

### 1.2 不是什么

- **不是真正的商业产品**——是单次演示用的产品原型
...
│   └── demo_scripts.md                     # 演示剧本 + 真 LLM 增强方案（已就绪 v1.0）
├── src/
│   ├── main.tsx
│   ├── App.tsx                     # 顶层布局 + 路由
...
**B 端工具风**，刻意远离 C 端互联网产品的活泼感。参考工业软件 / ERP 厂商的工业级调色（沉稳蓝灰为主调）。

### 4.2 色板

```typescript
const colors = {
  primary: '#3B5C7E',      // 主蓝灰（工业风沉稳基调）
...
以下是产品 PM 已经拍板的设计，**Claude Code 不允许自己改这些判断**。如果遇到这些判断在工程上"不好实现"，**停下来询问**，不要自作主张简化。
...
| PD-9 | 通知形态用**任务卡**（B 端 SaaS 主流术语），不叫"消息"或"通知" | UI 文案严格使用"任务卡" |
...
| `docs/demo_scripts.md` | 演示剧本 + 真 LLM 增强方案 | ✅ 已就绪 v1.0（FAQ + 4 个 LLM 接入点 + 演示节奏控制） |

---

## 13. 协作约定

Maintainer 的工作模式偏好（基于过往会话沉淀）：

- **诚实优先**：技术做不到的事情坦率说，不要硬编代码
- **少而精**：宁可问清楚再写，不要写一堆需要返工的代码
- **拍板权在 Maintainer**：所有产品判断由 Maintainer 决定，Claude Code 是工程执行者
- **可演示优先于完美**：第一目标是演示当天能跑，不是工程质量满分
...
- ❌ 用英文写本应是中文的 UI 文案（目标观众是中文 B 端用户，UI 全中文）
